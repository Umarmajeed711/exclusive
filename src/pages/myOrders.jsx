...
<div className="sticky top-0 z-40 backdrop-blur bg-gray-50/80 py-4">

  <div className="flex gap-3 mb-8 bg-white p-2 rounded-xl shadow-sm w-fit">
  {[
  ...
  {filteredOrders.length === 0 ? (
        ...
        <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filteredOrders.map((order) => {
            const isActive = ["pending", "processing", "shipped"].includes(order.delivery_status);
...
const isCancelled = order.delivery_status === "cancelled";
  ...
  {(isCompleted || isCancelled) && (
    <button
      ...
      className="px-3 py-2.5 bg-green-500 text-white rounded-lg hover:bg-green-600 transition"
    >
      Reorder
    </button>
  )}
